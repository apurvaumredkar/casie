 ...
 *
 * Skips to the previous track in the user's playback history.
 */
...
export async function handlePrevious(interaction: DiscordInteraction, env: Env): Promise<Response> {
  ...
  try {
    ...
    return messageResponse('⏮️ Skipped to previous track!');
  } catch (error) {
    ...
    return messageResponse('❌ Failed to skip to previous track. Please try again.', true);
  }
}
