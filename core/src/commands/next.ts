...
export async function handleNext(interaction: DiscordInteraction, env: Env): Promise<Response> {
  ...
  try {
    ...
    return messageResponse('⏭️ Skipped to next track!');
  } catch (error) {
    ...
    return messageResponse('❌ Failed to skip to next track. Please try again.', true);
  }
}
